...
# Gaussian distribution's parameters for 'actual' demands
mu1 = 300; sd1 = 50
mu2 = 100; sd2 = 10  
mu3 = 30; sd3 = 5  
...
set.seed(42)
d1 = demand.from.Gaussian(N.val,mu1,sd1)
d2 = demand.from.Gaussian(N.val,mu2,sd2)
d3 = demand.from.Gaussian(N.val,mu3,sd3)
OOS = cbind(d1,d2,d3)
...
for( n in N0:N.max ) {
  ...
  d1 = demand.from.Gaussian(n,mu1,sd1)
  d2 = demand.from.Gaussian(n,mu2,sd2)
  d3 = demand.from.Gaussian(n,mu3,sd3)
  S = cbind(d1,d2,d3)
...
if( !dir.exists(paste0("experiments_multi/Gaussian_OOS_",N.val,"_Size_",N.max)) )
    dir.create(paste0("experiments_multi/Gaussian_OOS_",N.val,"_Size_",N.max))
saveRDS( list(Ms=Ms,cs=cs,ss=ss,r=r,N.val=N.val,N.max=N.max,N0=N0,
              mu1=mu1,sd1=sd1,mu2=mu2,sd3=sd3,OOS=OOS,
              S.BO=S.BO,BO.init=BO.init,BO.iter=BO.iter,ker=ker,beta=beta,
              df0=df0,df_=df_,df.r=df.r,df.bo=df.bo),
         paste0("experiments_multi/Gaussian_OOS_",N.val,"_Size_",N.max,"/Gaussian_",N.val,"_Size_",N.max,"_Seed_",seed,".RDS") )
