...
# Gaussian distribution's parameters for 'actual' demand
mu = 300; sd = 50  
...
set.seed(42)
OOS = demand.from.Gaussian(N.val,mu,sd)
...
for( n in N0:N.max ) {
  ...
  S = demand.from.Gaussian(n,mu,sd)
  # solving w.r.t SAA
...
if( !dir.exists(paste0("experiments/Gaussian_OOS_",N.val,"_Size_",N.max)) )
    dir.create(paste0("experiments/Gaussian_OOS_",N.val,"_Size_",N.max))
saveRDS( list(M=M,c=c,s=s,r=r,N.val=N.val,N.max=N.max,N0=N0,
         ...
         paste0("experiments/Gaussian_OOS_",N.val,"_Size_",N.max,"/Gaussian_",N.val,"_Size_",N.max,"_Seed_",seed,".RDS") )
