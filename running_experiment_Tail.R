...
tail.on = "right" # admissible values: "right", "left", and "both"
mu = 500; sd = 50; alpha=0.15
...
set.seed(42)
OOS = demand.from.tail.distr(N.val,mu,sd,alpha,tail.on)
...
# S.BO = round(runif(100*N0,0,M))
# S.BO = unique(S.BO)
# S.BO = S.BO[1:N0]
#***********************************************************************

# if seed==19 the following while loop becomes infinite!
if( seed!=19 ) {
  S.BO = round(sample(pdf$x,N0,replace=T,prob=pdf$y))  
} else {
  S.BO = round(sample(pdf$x,100*N0,replace=T,prob=pdf$y))  
...
for( n in N0:N.max ) {
  ...
  S = demand.from.tail.distr(n,mu,sd,alpha,tail.on)
  # solving w.r.t SAA
...
if( !dir.exists(paste0("experiments/TailOn_",tail.on,"_OOS_",N.val,"_Size_",N.max)) )
    dir.create(paste0("experiments/TailOn_",tail.on,"_OOS_",N.val,"_Size_",N.max))
saveRDS( list(M=M,c=c,s=s,r=r,N.val=N.val,N.max=N.max,N0=N0,
         ...
         paste0("experiments/TailOn_",tail.on,"_OOS_",N.val,"_Size_",N.max,"/TailOn_",tail.on,"_",N.val,"_Size_",N.max,"_Seed_",seed,".RDS") )
