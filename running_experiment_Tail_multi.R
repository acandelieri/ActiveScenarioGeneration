...
seed = 17  # "seed", it also denotes the identifier of the independent run.
...
# Tail distribution's parameters for 'actual' demand
tail.on = "left" # admissible values: "right", "left", and "both"
mu1 = 300; sd1 = 50; alpha1=0.15
mu2 = 100; sd2 = 10; alpha2=0.15
mu3 = 30; sd3 = 5; alpha3=0.15
...
set.seed(42)
d1 = demand.from.tail.distr(N=N.val,mu=mu1,sd=sd1,alpha=alpha1,tail.on=tail.on)
d2 = demand.from.tail.distr(N=N.val,mu=mu2,sd=sd2,alpha=alpha2,tail.on=tail.on)
d3 = demand.from.tail.distr(N=N.val,mu=mu3,sd=sd3,alpha=alpha3,tail.on=tail.on)
OOS = cbind(d1,d2,d3)
...
for( n in N0:N.max ) {
  ...
  d1 = demand.from.tail.distr(n,mu1,sd1,alpha1,tail.on)
  d2 = demand.from.tail.distr(n,mu2,sd2,alpha2,tail.on)
  d3 = demand.from.tail.distr(n,mu3,sd3,alpha3,tail.on)
  S = cbind(d1,d2,d3)
...
if( !dir.exists(paste0("experiments_multi/TailOn_",tail.on,"_OOS_",N.val,"_Size_",N.max)) )
    dir.create(paste0("experiments_multi/TailOn_",tail.on,"_OOS_",N.val,"_Size_",N.max))
saveRDS( list(Ms=Ms,cs=cs,ss=ss,r=r,N.val=N.val,N.max=N.max,N0=N0,
              mu1=mu1,sd1=sd1,alpha1=alpha1,
              mu2=mu2,sd2=sd2,alpha2=alpha2,
              mu3=mu3,sd3=sd3,alpha3=alpha3,
              OOS=OOS,
              S.BO=S.BO,BO.init=BO.init,BO.iter=BO.iter,ker=ker,beta=beta,
              df0=df0,df_=df_,df.r=df.r,df.bo=df.bo),
         paste0("experiments_multi/TailOn_",tail.on,"_OOS_",N.val,"_Size_",N.max,"/TailOn_",tail.on,"_",N.val,"_Size_",N.max,"_Seed_",seed,".RDS") )
